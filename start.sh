...
if [ ! -f ".env" ]; then
  ...
  echo "    HEDERA_ACCOUNT_ID   — from https://portal.hedera.com"
  echo "    HEDERA_PRIVATE_KEY  — DER-encoded key from Hedera portal"
  echo "    JWT_SECRET          — any long random string (e.g. run: openssl rand -hex 32)"
  echo "    ANTHROPIC_API_KEY   — from https://console.anthropic.com (for AI vision features)"
  echo ""
  echo -e "  ${YELLOW}Optional (for full decentralized storage):${RESET}"
  echo "    BETHELNET_API_KEY   — from https://bethelnet.io"
  echo ""
